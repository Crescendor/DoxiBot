...
export const db = {
    ...
    // 20-Line system - store grid and winning lines
    async startSlotGameWithLines(channelId, userId, username, betAmount, grid, winningLines, totalWin) {
        const cid = String(channelId);
        await pool.query(
            `INSERT INTO slot_active_game (channel_id, user_id, username, bet_amount, total_spins, result, current_spin, current_multiplier)
             VALUES ($1::BIGINT, $2::BIGINT, $3, $4, 1, $5, 0, $6)
             ON CONFLICT (channel_id) DO UPDATE SET
             user_id = $2::BIGINT, username = $3, bet_amount = $4, total_spins = 1,
